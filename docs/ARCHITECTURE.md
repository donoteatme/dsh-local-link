...
The shipped Harness root and AppFrame own `sidebar`, `main`, `rightbar`, and `shell.overlay`; Conversation remains the native `main.conversation` panel. Local Link contributes only additive overlay, session-information, and subagent surfaces, then applies scoped responsive geometry to the existing frame. It never enumerates Chat, Trajectory, composer, dock, node, rightbar panel, or third-party registrations, so those continue to resolve through the native dynamic slots.
...
Harness `0.1.5-rc.1` does not expose a complete public responsive-shell or Drawer contract, spacing scale, radius scale, or mobile component recipes. Mobile geometry—safe-area placement, drawer widths, tap-target dimensions, spacing, radii, and transition timing—therefore remains plugin-owned CSS. One plugin-scoped `--dllm-side-drawer-width` value keeps the stock sidebar projection, the rightbar surface, and Current session visually identical instead of duplicating width literals. A small set of concepts also has no exported Harness primitive: Local access/device, navigation, current-session information, permission variants, and the subagent group glyph. The compact feedback-note affordance is a scoped presentation adapter because the stock feedback action accepts text only and exposes no icon-mode contract.
...
The plugin develops against the exact DeepSeek Harness `0.1.5-rc.1` contracts and separately checks `0.1.5-rc.2`. Most integration uses declared package contracts, but several adapters are intentionally version-sensitive:
